...
@section('content')
<h1 class="text-2xl font-semibold mb-6">Add Package</h1>
<form method="POST" action="{{ route('admin.packages.store') }}" class="bg-white rounded-lg shadow p-6 max-w-md">
    @csrf
    <div class="space-y-4">
        <div>
            <label for="coin_amount" class="block text-sm font-medium text-gray-700 mb-1">Coins</label>
            <input type="number" name="coin_amount" id="coin_amount" min="1" value="{{ old('coin_amount') }}" required class="w-full border border-gray-300 rounded px-3 py-2">
        ...
        <div>
            ...
            <input type="number" name="price_in_inr" id="price_in_inr" min="0" step="0.01" value="{{ old('price_in_inr') }}" required class="w-full border border-gray-300 rounded px-3 py-2">
